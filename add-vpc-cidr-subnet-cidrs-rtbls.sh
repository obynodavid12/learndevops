...
VPC_ID="vpc-xxxxx"
REGION="us-east-1"
NEW_VPC_CIDR="172.32.86.0/24"              # CIDR to add to the VPC
SOURCE_ROUTE_TABLE_ID="rtb-xxxxxxx # Copy routes from this table

# Naming configuration
NAME_PREFIX="usw2-vpc"                      # Prefix for resource names
SUBNET_TYPE="Private Subnet"                # e.g., "Private Subnet", "Public Subnet", "Database Subnet"
ROUTE_TYPE="Private Routes"                 # e.g., "Private Routes", "Public Routes", "Database Routes"

# Define your two subnet CIDRs here
SUBNET_1_CIDR="172.32.86.0/26"
SUBNET_2_CIDR="172.32.86.64/26"
...
create_subnet_with_routes() {
  ...
  # Extract AZ number from the AZ name (e.g., us-east-1a -> 1, us-east-1b -> 2)
  local AZ_SUFFIX=$(echo "$AZ" | grep -o '[a-z]
...
main)
  ...
  # Build names with format: usw2-vpc-Private Subnet (AZ1)
  local SUBNET_NAME="${NAME_PREFIX}-${SUBNET_TYPE} (AZ${AZ_NUMBER})"
  local RTB_NAME="${NAME_PREFIX}-${ROUTE_TYPE} (AZ${AZ_NUMBER})"
